        ...
        <div className="relative scroll-smooth font-sans">
            ...
            <section className="flex justify-center flex-wrap gap-6 mt-16 px-4">
                {heritageSites.map((site, index) => (
                    <div
                        key={index}
                        className="relative w-[100px] h-[320px] rounded-[50px] overflow-hidden cursor-pointer transition-transform duration-300 hover:scale-105"
                    >
                        <img
                            src={site.image}
                            alt={site.name}
                        ...
                        />
                        <span className="absolute bottom-2 left-1/2 transform -translate-x-1/2 -rotate-90 text-white font-bold text-sm shadow-md">
              {site.name}
            </span>
                    </div>
                ))}
